...
class Tracking(models.Model):
    ...
    startup = models.ForeignKey(
        "startups.Startup",  # adjust app label
        on_delete=models.CASCADE,
        null=True,
        blank=True,
        related_name="tracking_entries",
...
class Meta:
    ...
    @property
    def target_id(self):
        target = self.project or self.startup
    ...
    def clean(self):
        ...
        if has_project and has_startup:
            raise ValidationError(
                "Set only one of project or startup, not both."
            )
        if not has_project and not has_startup:
            raise ValidationError(
                "Set exactly one of project or startup."
            )
        ...
        elif self.target_type == TrackingTargetType.STARTUP:
            if not has_startup or has_project:
                raise ValidationError(
